package hlc.ud04.actividad401.auth.otp;

import java.security.InvalidParameterException;

import hlc.ud04.actividad401.auth.hash.AlgoritmoHash;
import hlc.ud04.actividad401.auth.hash.AlgoritmoHashSHA1;
...
public class GeneradorHOTP {
	...
	private String getDigitosDesdeHash(byte[] hash) {
		// Obtenemos el byte desde el que comenzar a partir de los 4 bits menos
		// significativos
		...
		// Nos quedamos con los n digitos (decimales) menos significativos del valor
		// truncado
		truncado %= Math.pow(10, digitos);

		// Se rellena con ceros en caso de que el número obtenido no tenga los dígitos
		// deseados
	...
	private byte[] longToByteArray(long valor) {
		...
		for (int i = 8; i-- > 0; valor >>>= 8) {
			// Se almacena el byte menos significativo en el elemento del array que le
			// corresponde
