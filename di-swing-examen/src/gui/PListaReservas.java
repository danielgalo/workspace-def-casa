...
public class PListaReservas extends JDialog {
	...
	public PListaReservas() {
		...
		}
		{
			...
			{
				JLabel lblFiltra = new JLabel("Clase");
	...
	private void iniciaTabla(JScrollPane scrollPane) {
		...
		for (Reserva reserva : GymPicassoMain.reservas) {
			Object[] fila = { reserva.getUsuario().getNombre(), reserva.getUsuario().getApellidos(),
					reserva.getClase().getNombreClase(), reserva.getClase().getTurno() };
			modelo.addRow(fila);
		}
